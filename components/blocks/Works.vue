      ...
      <div class="d-flex flex-row justify-content-start rounded border p-10">
        <ul class="nav border-transparent flex-column justify-content-start border-0 me-5 mb-3 mb-md-0 fs-6 pe-10"
          ...
          <li class="nav-item me-0 mb-md-2" v-for="sphere in spheres.data">
            <a :class="'nav-link text-gray-500 text-active-dark p-0' + (isActive(sphere.id) ? ' active' : '')"
            ...
            <div :class="'tab-pane fade ' + (isActive(sphere.id) ? 'show active' : '')" :id="sphere.slug" role="tabpanel">
              <div class="d-flex flex-wrap" role="tablist">
                <template v-for="work in sphere.works">
                  <a :class="'nav-link text-gray-500 p-2 m-1 text-active-dark pe-auto' + (isActiveWork(work.id) ? ' active' : '')"
                    data-bs-toggle="tab" :data-bs-target="'#sub' + work.id">
                    <span style="cursor: pointer;" class="mb-2 p-2 rounded bg-secondary ">{{ work.short_title }}</span>
                  </a>
                </template>
                    ...
                    <div :class="'tab-pane fade ' + (isActiveWork(work.id) ? 'show active' : '')" :id="'sub' + work.id"
                      ...
                      <div class="row">
                        <div class="col-6 d-flex flex-column ">
                          <div class="border border-info rounded p-1 bg-light shadow-sm mb-3">
                        ...
                        <div class="col-6 d-flex flex-column">
                          ...
                          <div class="border border-info rounded p-1 bg-light shadow-sm mb-3">
                            <h3 class="">Стоимость проекта</h3>
                            <p class="">{{ work.price_text }}</p>
                          </div>
                        </div>
                      </div>
                    </div>
