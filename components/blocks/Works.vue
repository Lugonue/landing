...
<template>
  <div class="container-xl d-flex justify-content-center border-bottom border-top rounded"
    style="margin-top: 10rem; padding-bottom: 10rem;" id="bg">
    <section class="col-lg-12" v-if="!pending">
      <h1 class="fs-2hx text-center fw-bold mt-10">Поиск работ</h1>
      <div class="d-flex flex-row justify-content-start p-10" id="mobile">
        <ul class="nav border-transparent flex-column justify-content-start border-0 me-5 mb-3 mb-md-0 fs-6 pe-10"
          ...
          <li class="nav-item me-0 mb-md-2" v-for="sphere in spheres.data" :key="sphere.id">
            <a :class="'nav-link text-gray-600 bg-active-light rounded p-1 text-active-dark' + (isActive(sphere.id) ? ' active' : '')"
            ...
            <div :class="'tab-pane fade ' + (isActive(sphere.id) ? 'show active' : '')" :id="sphere.slug" role="tabpanel">
              <div class="d-flex flex-wrap" role="tablist">
                <template v-for="work in sphere.works" :key="work.id">
                  <a :class="'nav-link rounded text-gray-600 p-2 m-1 text-active-dark pe-auto bg-active-light cursor-pointer' + (isActiveWork(work.id) ? ' active' : '')"
                    ...
                    <div :class="'tab-pane fade ' + (isActiveWork(work.id) ? 'show active' : '')" :id="'sub' + work.id"
                      ...
                      <div class="row">
                        <div class="col-lg-6 col-sm-12  d-flex flex-column ">
                          <div class="rounded p-4 bg-light shadow-sm mb-3">
